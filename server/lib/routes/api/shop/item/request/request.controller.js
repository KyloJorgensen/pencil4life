...
// Creates ItemRequest.
exports.createItemRequest = function (req, res, next) {
    var changes = {
        _itemId: '',
        quantity: 0,
    ...
    var query = request_model_1.ItemRequest.create(changes);
...
exports.getItemRequest = function (req, res, next) {
    var query;
    var adminfields = [];
    ...
    if (sessionUserId) {
        user_model_1.User.findOne({ _id: req.params._itemRequestId }, function (error, user) {
            ...
            query = request_model_1.ItemRequest.findOne(conditions);
        });
    }
    else {
        query = request_model_1.ItemRequest.findOne({ _id: req.params._itemRequestId, _sessionId: req.session.id });
    }
...
exports.getItemRequests = function (req, res, next) {
    ...
    new Promise(function (resolve, reject) {
        if (sessionUserId) {
            user_model_1.User.findById(sessionUserId)
                ...
                resolve(request_model_1.ItemRequest.find(conditions));
            });
        }
        else {
            resolve(request_model_1.ItemRequest.find({ _sessionId: req.session.id }));
        }
    }).then(function (query) {
...
exports.updateItemRequest = function (req, res, next) {
    var sessionUserId = op.get(req, ['session', '_userId'], null);
    ...
    new Promise(function (resolve, reject) {
        ...
        .then(function () {
        return request_model_1.ItemRequest.findOneAndUpdate(conditions, { $set: changes }, { new: true })
